...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO users (email, username, password, balance, is_active) VALUES ($1, $2, $3, $4, $5) RETURNING id',
      [email, username, hash, 0, false]
    );
...
app.get('/api/user/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const user = await pool.query('SELECT id, email, username, balance, is_active FROM users WHERE id = $1', [id]);
